...
#				'and max_num: ','%d' % max_num )
	...
	for en_id in range(int(args.en_num)):
		...
		n_hidden = (int(en_id)+1) * data_vec_size  # allocated heuristically
...
#	d_layer3_act = tf.nn.dropout(layer3_act, dropout)
		pred = last_layer = tf.add(tf.matmul(layer3_act, weights['h4']), biases['out'])
...
#		print('%d'%len(infer_results[0]))
	for i in range(len(infer_results[0])):
		...
		# get the average of the remaining outputs
			...
			output = summed_output / len(each_model_outputs)
