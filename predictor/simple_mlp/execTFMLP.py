...
# Hyper-Parameters
	if args.checkpoint_dir == "new":
		starter_learning_rate=0.001 # first training
	else:
		starter_learning_rate=0.001 # not first
	max_training_epochs = 20000
	batch_size = 64
...
#	layer3_act = tf.nn.relu(layer3)
	d_layer2_act = tf.nn.dropout(layer2_act, dropout)
	pred = tf.add(tf.matmul(d_layer2_act, weights['h4']), biases['out'])
...
#learning_rate = starter_learning_rate
	learning_rate = tf.train.exponential_decay(starter_learning_rate,global_step,max_training_epochs/10,0.95,staircase=False)
...
#				batch_x, batch_y = getRandBatch(val_data,val_label,len(val_label))
		...
		input_vec, label_vec, output_vec = sess.run([x,y,pred],feed_dict={x: batch_x, y: batch_y})
