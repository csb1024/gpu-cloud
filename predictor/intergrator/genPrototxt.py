...
def randomizePoolingLayer(layer, new_data_shape):
	...
	new_data_shape[2] = (new_data_shape[2] + - new_kernel_size)/new_stride + 1
	input_vector = []
...
def randomizeConvLayer(layer,new_data_shape):
	...
	new_data_shape[2] = (new_data_shape[2] + 2*new_pad - new_kernel_size)/new_stride + 1
...
def randomizeLayers(net_prototxt,layer_type):
	input_data_shape = []
	input_data_shape.append(random.randrange(2,129)) #num_of_batch
	input_data_shape.append(3) # new num of channel, Assume RGB channel size
	input_data_shape.append(random.randrange(24,225)) #new height/width , I think it is safe to assume data is going to be given in a square shape
