...
def parse_args():
	...
	parser.add_argument('phase',
			help='phase of neural network, forward OR backward')
	...
	with tf.Session(config=config) as sess:
		...
		for en_id in range(int(args.en_num)):
			...
			if (args.phase == 'backward') : 
				checkpoint_dir=args.chk_dir+"/"+args.layer_type+"-checkpoint-"+str(en_id)
...
#			print(label_vec)
			np.savetxt(args.gpu_dir+"/"+'input_vec.txt',input_vec,fmt='%.4e',delimiter=',')
#		print(len(label_vec))
