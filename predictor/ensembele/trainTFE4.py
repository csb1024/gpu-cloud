...
# Store layers weight & bias
	...
	'h5': tf.Variable(tf.random_normal([n_hidden,n_output]),name="h4"),
	'h6': tf.Variable(tf.random_normal([n_hidden,n_output]),name="h4"),
	'h7': tf.Variable(tf.random_normal([n_hidden,n_output]),name="h4"),
	'h8': tf.Variable(tf.random_normal([n_hidden,n_output]),name="h4")
	...
	'b4': tf.Variable(tf.random_normal([n_hidden]),name="b3"),
	'b5': tf.Variable(tf.random_normal([n_hidden]),name="b3"),
	'b6': tf.Variable(tf.random_normal([n_hidden]),name="b3"),
	'b7': tf.Variable(tf.random_normal([n_hidden]),name="b3"),
...
# Create model# Hidden layer with RELU activation
	...
	layer4 = tf.add(tf.matmul(layer3_act, weights['h4']), biases['b4'])
	layer4_act = tf.nn.relu(layer4)
	layer5 = tf.add(tf.matmul(layer4_act, weights['h5']), biases['b5'])
	layer5_act = tf.nn.relu(layer5)
	layer6 = tf.add(tf.matmul(layer5_act, weights['h6']), biases['b6'])
	layer6_act = tf.nn.relu(layer6)
	layer7 = tf.add(tf.matmul(layer6_act, weights['h7']), biases['b7'])
	layer7_act = tf.nn.relu(layer7)

	d_layer7_act = tf.nn.dropout(layer7_act, dropout)
	pred = last_layer = tf.add(tf.matmul(d_layer7_act, weights['h8']), biases['out'])
...
#	cost = loss = tf.sqrt(tf.reduce_mean(tf.square(tf.subtract(y, pred))))
...
#				print("Validation Error=",'{:.9f}'.format( error[0]))
		...
		print("Final Validation Error=",'{:.9f}'.format( fin_error))
