...
PROFILE_FLAG='f' # b : backward, f : forward
...
CAFFE_LOG_PARSER='parseCaffe2.py' 

#erase previously created data
rm $GENERATOR_DIR/$OUTPUT_VEC
rm $GENERATOR_DIR/$INPUT_VEC
...
#./analyzeCaffe_kernel4.sh $GENERATOR_DIR/data_generator.prototxt $layer $NVPROF_LOG_DIR $CAFFE_LOG_DIR $GPU_ID $profile 
...
# parse caffe log for time

# execute 
#$CAFFE train --solver $GENERATOR_DIR/data_generator.prototxt 1>/dev/null 2>$CAFFE_LOG_DIR/$CAFFE_LOG -gpu $GPU_ID

$CAFFE time -model $GENERATOR_DIR/$RAND_PROTOTXT -gpu $GPU_ID -iterations 10 1>/dev/null 2>$CAFFE_LOG_DIR/$CAFFE_LOG

#parse caffe log for duration of tasks and memory used 
$PYTHON $GENERATOR_ROOT_DIR/$CAFFE_LOG_PARSER $CAFFE_LOG_DIR/$CAFFE_LOG $layer_name $GENERATOR_DIR/$OUTPUT_VEC $PROFILE_FLAG
