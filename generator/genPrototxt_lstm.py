...
def randomizeDummyDataLayer(layer,new_data_shape):
	...
	elif "cont" in layer.name:
		dummy_shape[0].dim[0]=new_data_shape[0]
		dummy_shape[0].dim[1]=new_data_shape[1]
	elif "y" in layer.name:
		dummy_shape[0].dim[0]=new_data_shape[0]
		dummy_shape[0].dim[1]=new_data_shape[2]
...
def randomizeLSTMLayer(layer, new_data_shape):
	input_vector = []
	...
	params.num_output = new_num_output
...
def randomizeLayers(net_prototxt,layer_type):
	...
	# assuming lstm later accepts 1-d data, must change in future
	input_data_shape.append(random.randrange(10,50))
	input_data_shape.append(random.randrange(10,50))
	input_data_shape.append(random.randrange(8,200))
	...
	for layer in net_prototxt.layer:
		if layer.type == "DummyData":
			...
			input_vector = randomizeLSTMLayer(layer, input_data_shape)
			if layer_type == "LSTM":
				return_vec = input_vector
	return return_vec
